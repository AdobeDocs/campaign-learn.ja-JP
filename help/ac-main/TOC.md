...
+ コンテンツ制作 {#content-creation}
   + メール {#email}
      ...
      + {hide-from-toc}[AMP を使用したインタラクティブなメールコンテンツの定義](/help/content-creation/design-interactive-email-content-with-amp.md)
...
+ メッセージの送信{#sending-messages}
   ...
   + プッシュ {#push}
      ...
      + {hide-from-toc}[Experience Platform Mobile SDK を使用したプッシュチャネルの設定](/help/send-messages/mobile/configure-push-using-aep-mobile-sdk.md)
...
+ プロセス管理と自動化{#process-management}
   ...
   + 承認と検証のワークフロー {#create-approvals-and-validation-workflows}
...
+ 他のソリューションとの連携 {#connect}
   ...
   + {hide-from-toc}[Microsoft Dynamics 365 との統合](/help/connect/dynamics365-integration.md)
+ レポート {#reporting}
...
+ 管理 {#administration}
   + [ユーザー権限の管理](/help/administration/manage-user-permissions.md)
